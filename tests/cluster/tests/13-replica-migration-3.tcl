...
# multiple replicas attached, even though slaves are less than barrier settings.
...
test "Set barrier to 2 in all the instances" {
    foreach_redis_id id {
        R $id config set cluster-migration-barrier 2
    }
}
...
test "Restarting the previously killed slaves" {
    restart_instance redis 5
    restart_instance redis 10
    restart_instance redis 6
    restart_instance redis 11
}

test "Each master should have two replicas again" {
    foreach_redis_id id {
        if {$id < 5} {
            wait_for_condition 1000 50 {
                [llength [lindex [R 0 role] 2]] == 2
